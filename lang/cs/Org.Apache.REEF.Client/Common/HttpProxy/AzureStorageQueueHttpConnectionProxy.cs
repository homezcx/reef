...
namespace Org.Apache.REEF.Client.DotNet.AzureBatch.HttpProxy
{
    ...
    /// </summary>
    public class AzureStorageQueueHttpConnectionProxy
    {
        private static readonly Logger LOGGER = Logger.GetLogger(typeof(AzureStorageQueueHttpConnectionProxy));
        ...
        public AzureStorageQueueHttpConnectionProxy(string storageAccountName, string storageAccountKey, string queueName)
        {
        ...
        public async Task<HttpProxyResponseProto> RequestMessageResponseAsync(HttpProxyRequestProto request)
        {
            ...
            await _queue.AddMessageAsync(new CloudQueueMessage(Convert.ToBase64String(request.Serialize())));
            AzureStorageQueueHttpProxyReceiver receiver = new AzureStorageQueueHttpProxyReceiver(_storageAccountName, _storageAccountKey, request.responseQueue);
